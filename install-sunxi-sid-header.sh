...
LINUX_HEADER="${HEADER_ROOT}/include/linux/sunxi-sid.h"
COMPAT_HEADER="${HEADER_ROOT}/include/sunxi-sid.h"
...
if [[ ! -d "$HEADER_ROOT" ]]; then
    echo "ERROR: Kernel header directory does not exist:"
    echo "  $HEADER_ROOT"
    echo
...
if [[ -e "$LINUX_HEADER" ]]; then
    BACKUP="${LINUX_HEADER}.backup.$(date +%Y%m%d-%H%M%S)"
    cp -a "$LINUX_HEADER" "$BACKUP"
    echo "Existing header backed up to:"
    echo "  $BACKUP"
fi

cat > "$LINUX_HEADER" <<'HEADER'
/*
 * linux/sunxi-sid.h
 *
...
chmod 0644 "$LINUX_HEADER"

ln -sfn linux/sunxi-sid.h "$COMPAT_HEADER"

echo
echo "Installed:"
echo "  $LINUX_HEADER"
echo
echo "Compatibility link:"
ls -l "$COMPAT_HEADER"

echo
echo "Validation:"
grep -E \
    'sunxi_get_soc_ver|sunxi_get_soc_chipid|sunxi_get_serial' \
    "$LINUX_HEADER"
